...
void read_v_line()
{
	...
	char n='n', H='H', t = 't';
	sprintf(str_brut, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c", M,m,h,h,comma,space,p,u,z,z,l,e,d,escpoint,space,G,i,o,c,o,f,o,r,z,a,space,s,a,p,e,v,o,space,n,o,n,space,c,i,space,s,a,r,e,s,t,i,space,r,i,u,s,c,i,t,o,comma,space,H,a,h,a,h,h);
	puts(str_brut);
	fflush(stdout);
	char admpath[5];
	admpath[0] = 0x48;
...
void read_on()
{
	FILE *fd = fopen("graded_list.txt", "r");
	int i = 0;
	if(fd == NULL)
	{
	...
	for(i = 0; i < 583; i++)
		fscanf(fd, "%s %s %d", &grad_list[i].name, &grad_list[i].surname, &grad_list[i].score);
}
